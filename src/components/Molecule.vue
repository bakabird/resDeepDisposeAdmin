<template>
  <div class="bar molecule">
    <div>
      <input class='url' type="text" v-model="URL">
      <button class='spyGo' @click="fetchInfo">发动间谍进行调查</button>
    </div>
    <div>
    ...
    <div class="proConfig" v-if='proConfig'>
      <div class='proConfig_other'>
    ...
    <div class="tagWrap">
      ...
      <div v-for="(tag,key) in tags" :key="key + '_tag_group'" class="tag">
        {{key}}
    ...
    <div>
      <button @click="revise">[{{sqlId}}]完成编辑</button>
      <input class='index' type="number" v-model="INDEX">
    </div>
  </div>
</template>
